# tsukasamiyashita/dxfflowchartmiya/DxfFlowchartMiya/app.py
import sys
import os
import json
import uuid
...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QToolBar, QFileDialog, QMessageBox, QGraphicsItemGroup,
                             ...
                             QProxyStyle, QStyle, QDockWidget, QGraphicsScene, QGraphicsPathItem, 
                             QGraphicsTextItem, QGraphicsEllipseItem, QGraphicsLineItem, QTextEdit)
...
from PyQt6.QtGui import (QPen, QBrush, QColor, QPainter, QImage, QAction, QActionGroup,
                         QPageSize, QPageLayout, QUndoStack, QCursor, QIcon, QPainterPath, 
...
def import_dxf_as_items(path):
    """DXFファイルを読み込み、QGraphicsItem のリストとして返す（エラーで落ちないように堅牢化）"""
    try:
    ...
    for entity in msp:
        try:
            ...
            if entity.dxftype() == 'LINE':
                ...
                f = t_item.font()
                font_size = entity.dxf.height * 0.75 if hasattr(entity.dxf, 'height') else 10
            ...
            # 描画非対応やパースエラーのエンティティはスキップして処理を継続
            print(f"Skipping entity {entity.dxftype()} due to error: {e}")
            continue
...
class MainWindow(QMainWindow):
    ...
    def delete_selected_items(self):
        ...
        for i in sel:
            ...
            elif isinstance(i, DxfFrameItem): bgs.add(i)
